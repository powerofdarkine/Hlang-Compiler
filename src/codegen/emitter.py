...
class Emitter:
    ...
    def emit_return(self, in_, frame) -> str:
        ...
        if type(in_) is ArrayType or type(in_) is StringType :
            frame.pop()
    ...
    def clear_buff(self) -> None:
        ...
        """
        self.buff.clear()
    def emit_new_array(self, in_, frame):
        val = ""
        if type(in_) is IntType:
            val = "int"
        if type(in_) is FloatType:
            ...
            return self.emit_anew_array(in_, frame)
        return self.jvm.emitNEWARRAY(val)

    def emit_anew_array(self, in_, frame):
        val = ""
        if type(in_) is FloatType:
            val = "float"
        if type(in_) is IntType:
            ...
            val = self.get_jvm_type(in_)
        return self.jvm.emitANEWARRAY(val)
    def emitARRAYLENGTH(self):
        return JasminCode.INDENT + "arraylength" + JasminCode.END
